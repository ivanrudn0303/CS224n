...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros(n_features * embeddings.shape[1], hidden_size))
        torch.nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros(1, hidden_size))
        torch.nn.init.xavier_uniform_(self.embed_to_hidden_bias)
        ...
        self.hidden_to_logits_bias = nn.Parameter(torch.zeros(1, n_classes))
        torch.nn.init.xavier_uniform_(self.hidden_to_logits_bias)
    ...
    def embedding_lookup(self, w):
        ...
        ### END YOUR CODE
        return self.embeddings.index_select(0, w.view(-1)).view(w.shape[0], -1)
    ...
    def forward(self, w):
        ...
        out = self.dropout(self.embedding_lookup(w))
        hidden = self.relu(torch.matmul(out, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
