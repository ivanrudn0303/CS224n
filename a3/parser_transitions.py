...
class PartialParse(object):
    ...
    def finished(self):
        return self.buffer == [] and len(self.stack) == 1
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while partial_parses:
        unfinished_parses = partial_parses[:min(batch_size, len(partial_parses))]
        partial_parses = partial_parses[min(batch_size, len(partial_parses)):]
        while unfinished_parses:
            translations = model.predict(unfinished_parses)
            for idx in range(len(translations)):
                unfinished_parses[idx].parse_step(translations[idx])
            unfinished_parses = [parser for parser in unfinished_parses if not parser.finished()]
    ### END YOUR CODE
