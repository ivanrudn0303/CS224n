...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sents_len = max(len(s) for s in sents)
    sents_padded = []
    for sent in sents:
        sents_padded.append([word[:min(max_word_length, len(word))] + [char_pad_token] * (max_word_length - min(max_word_length, len(word))) for word in sent] + [[char_pad_token] * max_word_length] * (max_sents_len - len(sent)))
    ### END YOUR CODE
